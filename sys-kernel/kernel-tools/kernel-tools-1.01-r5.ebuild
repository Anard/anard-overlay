...
src_configure() {
	...
	if use grub; then
		sed -i '6s/.*/GRUB=1/' "${S}/build-kernel.cnf"
	else
		sed -i '6s/.*/GRUB=0/' "${S}/build-kernel.cnf"
	fi
	# set INITRAMFS variable in build-kernel.cnf
	if use genkernel; then
		sed -i '9s/.*/INITRAMFS=\'genkernel\'/' "${S}/build-kernel.cnf"
	elif use dracut; then
		sed -i '9s/.*/INITRAMFS=\'dracut\'/' "${S}/build-kernel.cnf"
	else
		sed -i '9s/.*/INITRAMFS=\'no\'/' "${S}/build-kernel.cnf"
	fi
}
